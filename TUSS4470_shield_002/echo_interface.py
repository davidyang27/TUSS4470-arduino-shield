...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        if self.large_depth_label.isVisible():
            self.large_depth_label.setText(f"{depth_cm / 100:.2f} m")
            
            if is_valid:
                # 綠色：數據可信
                self.large_depth_label.setStyleSheet("""
                    QLabel { color: #00FF00; font-size: 84px; font-weight: bold; }
                """)
            else:
                # 紅色：硬體與軟體偵測不符
                self.large_depth_label.setStyleSheet("""
                    QLabel { color: #FF0000; font-size: 84px; font-weight: bold; }
                """)
            # -----------------------
